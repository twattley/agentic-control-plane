# Ticket 027: The plane dispatches agents at a chosen reasoning effort
...
- New settings fields `builder_effort` / `reviewer_effort`
  ...
  already consumes.
- Detached workers receive both settings through the explicit child environment
  assembled in `apps/api/app/services/executor.py`.
...
- `depends_on`: none
- `parallelizable`: no — configuration, detached-process propagation, and
  command construction form one vertical seam and should move together.
...
- [x] With `builder_effort` set, a claude builder command includes
      `--effort <level>`, pinned by a test asserting the exact argv.
- [x] With `builder_effort` set, a codex builder command includes the
      confirmed `-c model_reasoning_effort=<level>` override, pinned by a test.
- [x] With effort unset, `_agent_command` emits no effort flag for any
      provider — the argv matches today's exactly.
- [x] `reviewer_effort` drives the reviewer command independently of
      `builder_effort`.
- [x] Detached workers receive both configured effort values in their explicit
      child environment, pinned by a test.
- [x] Setting effort on a `stub` provider is a no-op (no flag, no crash).
