...
async def create_run(conn: asyncpg.Connection, data: RunIn) -> Run:
    row = await conn.fetchrow(
        """
        INSERT INTO runs (repo_id, ticket_id, title, mode, builder_provider, reviewer_provider)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id, repo_id, ticket_id, title, mode, state,
                  builder_provider, reviewer_provider, created_at, updated_at
        """,
        data.repo_id, data.ticket_id, data.title, data.mode,
        data.builder_provider, data.reviewer_provider,
    )
    return Run(**dict(row))
...
async def get_run(conn: asyncpg.Connection, run_id: int) -> Run | None:
    row = await conn.fetchrow(
        """
        SELECT id, repo_id, ticket_id, title, mode, state,
               builder_provider, reviewer_provider, created_at, updated_at
